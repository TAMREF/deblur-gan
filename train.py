...
def train_multiple_outputs(n_images, batch_size, epoch_num, critic_updates=5, dodebug=False):
	...
	for epoch in range(epoch_num):
		...
		d_losses = []
		d_on_g_losses = []
		if dodebug : print('Flag #1 : Loop')
		if dodebug : print(x_train.shape)
		for index in range(int(x_train.shape[0] / batch_size)):
			if dodebug : print(index)
			batch_indexes = permutated_indexes[index*batch_size:(index+1)*batch_size]
			image_blur_batch = x_train[batch_indexes]
			image_full_batch = y_train[batch_indexes]
			if dodebug : print('Flag #2 : x_train, y_train')
			
			generated_images = g.predict(x=image_blur_batch, batch_size=batch_size)
			if dodebug : print('Flag #3 : g.predict')
			
			for i in range(critic_updates):
				if dodebug : print('Flag #4 : critic training : {}'.format(i))
...
def train_command(n_images, batch_size, epoch_num, critic_updates,dodebug):
	return train_multiple_outputs(n_images, batch_size, epoch_num, critic_updates,dodebug)
